...
class Trainer:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            main_loss,additional_loss,loss = self.compute_loss(model, inputs)
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            main_loss,additional_loss,loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        main_loss,additional_loss,loss, outputs = self.compute_loss(model, inputs, return_outputs=True)
        ...
        if prediction_loss_only:
            return (main_loss,additional_loss,loss, None, None)

        logits = nested_detach(logits)
        if len(logits) == 1:
            logits = logits[0]
        return (main_loss,additional_loss,loss, logits, labels)
