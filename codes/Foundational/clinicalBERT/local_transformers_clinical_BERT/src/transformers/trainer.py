...
class Trainer:
    ...
    def __init__(
        ...
        # additional_labels: Optional[List] = None,
    ):
        self.training_loss=[]
        self.training_loss_main=[]
        self.training_loss_additional=[]
        self.val_loss=[]
        self.val_loss_main=[]
        self.val_loss_additional=[]
        ...
        #############################################################################
        ## added this line to take additional_labels for semi-supervised learning ###
        #############################################################################
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            # this line seems neccessary because if not it throws an error, saying that it appears in multiple parameters
            # to verify it, i added a print statement (below) and intentional threw an error, and it does include the ['auxiliary.weight', 'auxiliary.bias'] terms
            optimizer_grouped_parameters = [
                {
                "params": [
                p for n, p in opt_model.named_parameters() if (n in decay_parameters or "auxiliary" in n) and p.requires_grad
                ],
                "weight_decay": self.args.weight_decay,
                },
                {
                "params": [
                p for n, p in opt_model.named_parameters() if (n not in decay_parameters and "auxiliary" not in n) and p.requires_grad
                ],
                "weight_decay": 0.0,
                },
                ]
            ...
            #     ]
            #w
            # Add the following lines to print parameter names
            #print("Decay parameters:", decay_parameters)
            #print("Non-decay parameters:", [n for n, p in opt_model.named_parameters() if n not in decay_parameters])
            #w
    ...
    def _inner_training_loop(
        ...
        #######################################################################################################################
        ### Adding binary_cross_entrophy_loss to the total loss, effectively making it a semi-supervision fine-tuning model ###
        #######################################################################################################################

        # logits = model(input_ids[j:j+1], attention_mask=attention_mask[j:j+1]).logits
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss,main_loss,additional_loss = self.compute_loss(model, inputs)
        ...
        if self.do_grad_scaling:
            ...
            loss.backward()
        #print("loss.backward() step,\ntotal loss:", loss.item(),"training loss:", main_loss.item(),"additional loss:", additional_loss.item())
        #w
        self.training_loss.append(loss.item())
        self.training_loss_main.append(main_loss.item())
        self.training_loss_additional.append(additional_loss.item())
        return loss.detach()
    ...
    #         print("self.label_smoother is not None") 
    #     if "labels" in inputs:
    #         print("labels in inputs") 
    ...
    #     else:
    #         print("check-point5:\nlabels is None... computing loss as: \"loss = outputs[loss] if isinstance(outputs, dict) else outputs[0]\"s\n")
    ...
    #     print("input looks like this", inputs)
    #     print("\n Labels look like this:", labels)
    #     print("\n outputs look like this", outputs)
    #     w # intentionally throwing an error
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss,main_loss,additional_loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            #w
            self.val_loss.append(loss.item())
            self.val_loss_main.append(main_loss.item())
            self.val_loss_additional.append(additional_loss.item())
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        loss, main_loss,additional_loss, outputs = self.compute_loss(model, inputs, return_outputs=True)
                    loss = loss.mean().detach()
                    main_loss=main_loss.mean().detach()
                    additional_loss=additional_loss.mean().detach()
        ...
        if prediction_loss_only:
            return (loss,main_loss,additional_loss, None, None)

        logits = nested_detach(logits)
        if len(logits) == 1:
            logits = logits[0]
        return (loss,main_loss,additional_loss, logits, labels)
